...
if __name__ == '__main__': 
    ...
    os.chdir(this_dir)
    with open(this_dir / 'yolo_parameters.yaml', 'r') as file:
        data = yaml.safe_load(file)
        if 'test' in data and data['test'] is not None:
            images_dir = Path(data['test']) / 'images'
        else:
            print("No test field found in yolo_parameters.yaml, please add the test field with the path to the test images")
            exit()
    ...
    data = this_dir / 'yolo_parameters.yaml'
